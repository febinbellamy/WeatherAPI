...
let weather = {
  ...
  extractLatLon(data) {
    ...
    fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&units=Imperial&exclude=minutely,hourly&appid=${this.apiKey}`)
      .then((res) => res.json())
      .then((oneCallData) => {
        ...
        document.querySelector(".temp").innerHTML = `Current Temperature: ${roundToNearestNum(temp)} °F`;
        document.querySelector(".feelslike").innerHTML = `Feels like ${roundToNearestNum(feels_like)} °F`;
        document.querySelector(".precipitation").innerHTML = `Chance of rain: ${precipitation * 100}%`;
      })
      .catch((e) => console.log("Error!", e));
  },
  search() {
